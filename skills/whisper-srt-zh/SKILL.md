...
**速度（2026-08-08 修正）**：獨佔 GPU 時 breeze25 約 **0.13 倍實時**——9 支共 264 分鐘的
講座只花 31 分鐘（RTX 4060 8GB，8 支中文 + 1 支英文，倍率 0.09–0.17 相當穩定）。
它沒有比 turbo 慢十倍，換預設幾乎不需要付速度代價。

==先前這裡寫「約 1.2 倍實時、慢十倍」是量錯的==：那兩個數字量於 GPU 同時被另一個轉錄
工作占用時。`asr-benchmark` 的第一條紀律就是「一個行程一個模型」，他們最初「Breeze 慢
2 倍」的結論正是同一個陷阱。==效能數字必須在獨佔 GPU 時量==，否則會得到相反的結論並
據此選錯引擎。

精確率的優勢不變：78.1% → 92.5%，省下的是**後面校正術語的人力**——一場講座裡 turbo 會把
mammogram 拼成七種變體，那些全部要人回頭對講義改。
