...
- **WHEN** `~/.lecture-to-notes/outputs.toml` sets note.style = "faithful" and `./.lecture-to-notes/outputs.toml` sets note.style = "concise"
...
- **WHEN** `./.lecture-to-notes/outputs.toml` contains a syntax error on line 7
...
The repository SHALL contain `examples/overlay-minimal/` with one valid instance of each overlay-able file using placeholder content, and the README SHALL describe how to copy it to `~/.lecture-to-notes/`.

#### Scenario: Example overlay is valid

- **WHEN** `examples/overlay-minimal/` is copied to `~/.lecture-to-notes/` and `l2n profile show` runs
