...
完整 rewritten candidate 必須以 canonical serialization 計算 `approved_candidate_sha256`，輸入固定包含 chapter identity、精確 start/end times、`title`、`summary_zh`、正好四項依 index 排序的 `takeaways_zh`、完整 `editorial_notes_zh`，以及按 content-unit identity 與 citation identity 決定性排序的完整 citation map。每個逐 claim 人工來源支持 attestation 必須綁定 normalized claim SHA-256、完整引用 identity/hash 集合、evidence packet SHA-256、此 `approved_candidate_sha256`、非空 reviewer identity，以及明確 `support_confirmed=true`。任何 title、summary、takeaway、editorial note、chapter identity/time 或 citation map 在核准後變更，都會改變 digest，禁止將舊 packet/review replay 到新 candidate。
...
- Rewrite SHALL default to manual reviewed imports. Each segment SHALL expose five speaker claim clauses (`summary_zh` plus four indexed `takeaways_zh`) and a sixth reviewed content unit (`title`). Every speaker clause, including unchanged legacy content, SHALL carry one or more typed trusted citations plus a human source-support attestation; a case-specific factual title SHALL meet the same requirement, while an unattested title SHALL be restricted to a non-case topical label. Each attestation SHALL bind `normalized_claim_sha256`, the complete citation identity/hash set, `evidence_packet_sha256`, `approved_candidate_sha256`, non-empty `reviewer_id`, and `support_confirmed=true`. `approved_candidate_sha256` SHALL be the deterministic canonical digest of chapter identity/times, title, summary, exactly four ordered takeaways, editorial notes, and the complete deterministically ordered citation map. External provider mode SHALL require `--allow-external-llm`, confirmation `TEXT-EVIDENCE-ONLY`, a clean sensitive-data scan, environment/profile credentials, and human review before apply.
...
- Evidence validation SHALL accept supported paraphrase without exact clause equality, but SHALL NOT infer semantic support mechanically. It SHALL fail closed for incomplete five-speaker-clause coverage, unsupported case-specific title assertions, absent or unrelated citations, missing attestations, packet-only frame trust, untrusted citation types, any reviewer/confirmation/type/hash/digest/binding tamper, or replay of an approved packet/review after changing title, summary, any takeaway, editorial notes, chapter identity/times, or citation map. Packet-internal hashes SHALL count only as integrity evidence; canonical frame trust SHALL come from an independently trusted external manifest. `editorial_notes_zh` SHALL remain separately labeled and excluded from speaker-claim evidence while remaining bound by `approved_candidate_sha256`.
