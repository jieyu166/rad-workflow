...
每章先由 SRT、正式選取影格 OCR 與既有內容建立帶 SHA-256 的 evidence packet，再由 manual-first rewrite 結果與 review record 套用。`summary_zh` 視為一個 speaker claim clause，正好四項 `takeaways_zh` 各視為一個 speaker claim clause，因此每章必須完整覆蓋五個 speaker clause；每個 `title` 都是第六個受 review 內容單元，rewrite candidate 必須明列 `title_kind`，且值只能是 `topical` 或 `case_claim`。`topical` 代表不主張個案病理、追蹤結果、病史、影像所見或診斷，title citation set 必須為空，並由 reviewer 明確設定 `topical_title_confirmed=true`；`case_claim` 則必須和 speaker clause 一樣具備一個以上可信引用與逐 claim `support_confirmed=true` attestation。未改寫的 legacy `existing_content` 也不得沿用信任，必須通過相同 gate。每個需支持的 clause 可重組或意譯，不要求與來源文字相等，但必須列出至少一個可信來源引用，且每個引用只能是 evidence packet 中具穩定 paragraph identity 與 SHA-256 的可信逐字稿段落，或由 packet 外部可信 canonical frame manifest 驗證 identity、asset hash、OCR/source hash 的正式影格來源。

`title_kind` 是人工語意分類，不得宣稱由 regex 或關鍵字完整判定。機械 red flags 可對病理斷言、追蹤狀態、既往病史或其他明顯個案語句強制要求 `case_claim`，但 red flag 未命中不能證明標題是 `topical`；具名 reviewer 仍須確認每個 `topical` 分類。缺少 `title_kind`、非字串、列舉外值、`topical` 缺確認或帶 title citations、`case_claim` 缺可信引用/attestation，以及分類在核准後被竄改或 replay，全部 fail closed。

Review 採非循環雙 digest。第一層 `approved_candidate_sha256` 對 canonical serialization 計算，輸入固定包含 chapter identity、精確 start/end times、`title`、`title_kind`、`summary_zh`、正好四項依 index 排序的 `takeaways_zh`、完整 `editorial_notes_zh`，以及完整 citation map；citation map 先按 content-unit identity 與 citation identity/hash 決定性排序，因此純輸入順序調整語意等價且 digest 不變，但新增、刪除或修改 citation identity/type/hash 必須改變 digest。第二層 `review_attestation_sha256` 的 canonical payload 固定包含 `approved_candidate_sha256`、`evidence_packet_sha256`、按 content-unit identity 決定性排序的逐 claim attestation（僅含 normalized claim SHA-256、order-insensitive canonical citation identity/hash set、`support_confirmed`）、經 Unicode NFC 與首尾空白移除的非空 `reviewer_id`、明確 `source_support_confirmed=true`、`case_facts_confirmed=true`、`editorial_separation_confirmed=true`、布林 `topical_title_confirmed`（且僅 `title_kind="topical"` 時必須為 `true`），以及 `review_schema="lecture-content-review"` 與整數 `review_version=1`。digest payload 不包含 `review_attestation_sha256` 自身，避免循環。

Approved run state 必須同時記錄並驗證 `approved_candidate_sha256` 與 `review_attestation_sha256`，且不得只靠 candidate digest 宣稱 review 有效。任何 candidate 欄位（包括 `title_kind`）或 citation membership 變更會破壞第一層 digest；任何 reviewer、confirmation、claim hash、citation membership/hash 或 `support_confirmed` 變更會破壞第二層 digest。純 citation 輸入順序調整在 canonicalization 後必須 PASS。禁止將舊 packet/review replay 到任一 digest 不符的 candidate 或 review record。

機械 validator 只驗完整 content-unit/claim coverage、引用存在與允許類型、外部 manifest trust、所有 hash/digest、reviewer、確認旗標及彼此 binding；不得以 exact/fuzzy text matching 宣稱語意等價或來源支持。缺少引用、attestation、可信來源或任一 binding mismatch 一律 fail closed；packet 內自帶 hash 只能證明 packet 完整性，不能自行建立來源真實性。findings 只能回報 segment/content-unit identity、錯誤 code 與欄位，不得回顯敏感 claim 或 source 文字。`editorial_notes_zh` 保持獨立標示，既不是 speaker clause，也不得作為病例 claim 的支持來源，但仍納入 full-candidate digest，防止 review 後被替換。
...
- Rewrite SHALL default to manual reviewed imports. Each segment SHALL expose five speaker claim clauses (`summary_zh` plus four indexed `takeaways_zh`) and a sixth reviewed content unit (`title`); each rewrite candidate SHALL carry explicit review metadata string `title_kind` equal to exactly `topical` or `case_claim`. Every speaker clause, including unchanged legacy content, SHALL carry one or more typed trusted citations plus a human source-support attestation. A `topical` title SHALL have no title citation or claim attestation and SHALL require digest-bound reviewer `topical_title_confirmed=true`; a `case_claim` title SHALL carry one or more trusted citations plus a per-claim `support_confirmed=true` attestation. Semantic title classification SHALL be human-reviewed; mechanical red flags SHALL be allowed to force `case_claim` but SHALL NOT be treated as exhaustive proof that an unflagged title is topical. `approved_candidate_sha256` SHALL digest chapter identity/times, title, `title_kind`, summary, exactly four ordered takeaways, editorial notes, and the complete citation map after order-insensitive canonical sorting. `review_attestation_sha256` SHALL separately digest `approved_candidate_sha256`, `evidence_packet_sha256`, canonical per-claim records (`normalized_claim_sha256`, canonical citation identity/hash set, `support_confirmed=true`), normalized non-empty `reviewer_id`, `source_support_confirmed=true`, `case_facts_confirmed=true`, `editorial_separation_confirmed=true`, boolean `topical_title_confirmed`, `review_schema="lecture-content-review"`, and integer `review_version=1`, without including itself. Approved run state SHALL bind and verify both digests. External provider mode SHALL require `--allow-external-llm`, confirmation `TEXT-EVIDENCE-ONLY`, a clean sensitive-data scan, environment/profile credentials, and human review before apply.
...
- Evidence validation SHALL accept supported paraphrase without exact clause equality, but SHALL NOT infer semantic support mechanically. It SHALL fail closed for incomplete five-speaker-clause coverage, missing/non-string/out-of-enum `title_kind`, `topical` without `topical_title_confirmed=true` or with title evidence, `case_claim` without trusted citations/attestation, title-classification tamper/replay, absent or unrelated citations, missing attestations, packet-only frame trust, untrusted citation types, any reviewer/confirmation/type/hash/digest/binding tamper, either approved-run digest missing/mismatched, or replay after changing title, summary, any takeaway, editorial notes, chapter identity/times, or citation membership/type/hash. Citation reordering alone SHALL PASS after order-insensitive canonicalization. Packet-internal hashes SHALL count only as integrity evidence; canonical frame trust SHALL come from an independently trusted external manifest. `editorial_notes_zh` SHALL remain separately labeled and excluded from speaker-claim evidence while remaining bound by `approved_candidate_sha256`.
...
- `tests.test_lecture_content_rules` proves supported paraphrase with bound transcript/frame evidence passes; generic `title_kind="topical"` titles pass only with `topical_title_confirmed=true` and no title evidence; implicit pathology, follow-up-status, and history titles require `title_kind="case_claim"`, trusted citations, and claim attestation; missing/wrong-type/out-of-enum/classification-tampered title metadata fails; exact unchanged legacy content without citation fails; unrelated citation, attestation mismatch, missing clause coverage, packet-only forged frame trust, and reviewer/confirmation/type/hash/digest tampering fail closed; independently manifested trusted frame evidence passes; replay after any title, editorial-note, summary, takeaway, chapter identity/time, or citation membership/type/hash change fails through `approved_candidate_sha256`; reviewer, confirmation, claim-attestation or review schema/version mutation fails through `review_attestation_sha256`; citation reordering alone preserves both canonical digests and passes; approved run state missing either digest fails; editorial notes stay separate; findings contain identities/codes only and never sensitive claim/source text.
...
- [Automated rules cannot prove medical truth, semantic equivalence, or exhaustive title classification] → Validator 僅證明 coverage、integrity、external trust 與 attestation binding；red flags 只可保守強制 `case_claim`，逐 title 分類與逐 claim 來源支持由具名 reviewer 明確確認，且 editorial separation 維持 hard gate。
