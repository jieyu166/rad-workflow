...
"""
    ...
    # ── 9. 時段效率（每整點熱力圖 + 平日 cases/hr 折線）──
    try:
        import tod_efficiency as tod
        from datetime import datetime as _dt
        tod_data = tod.compute(tod.all_csv_paths(), now=_dt(2026, 12, 31, 23, 59))
    except Exception as _e:
        tod_data = None
        print(f"  tod_efficiency skipped: {_e}")
    if tod_data:
        tod_heat = tod.heatmap_html(tod_data)
        tod_line = tod.linechart_svg(tod_data["weekday"])
        html += f"""
<!-- 9. 時段效率 -->
<div class="section">
  <div class="section-title"><span class="num">9</span>時段效率分析（每整點 0–23）</div>
  {tod_heat}
  <div class="card" style="margin-top:14px">
    <h4 style="font-size:13px;color:#fff;margin:0 0 8px">平日 cases/hr 折線（08–23）</h4>
    {tod_line}
    <div style="font-size:12px;color:var(--text-dim);margin-top:8px">
      高峰時段＝把高專注/高價值報告（Mammo、CT、困難 US）優先排入；低谷留給可被打斷的 XR 批次。
      cases/hr 由相鄰簽發間隔推估（&gt;20min 截斷），report_time 精度到分。
    </div>
  </div>
</div>
"""
